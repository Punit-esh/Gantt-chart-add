...
export const SideMenu = ({ setadd_modal, rel_data_arr, setrel_data_arr }) => {
  ...
  const handleFileUpload = (current) => {
    ...
    fr.onload = function () {
      ...
      if (flag) {
        ...
        setrel_data_arr(temp_arr);
        localStorage.setItem("rel_data_arr", JSON.stringify(temp_arr));
      }
      console.log(flag);
    };
      ...
      <div className="side_menu">
        ...
        <a
          href={
            ...
            btoa(
              JSON.stringify(
                rel_data_arr.length == 0 ? "no data available" : rel_data_arr
              )
            )
          }
